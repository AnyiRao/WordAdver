...
#    train_set_x, train_set_y = shared_dataset((train_set[:,:img_h],train_set[:,-1]))    
    ...
    Fdin_onehot=theano.function([index], din_onehot,
          givens={
            x: train_set_x[index*batch_size:(index+1)*batch_size],
              y: train_set_y[index*batch_size:(index+1)*batch_size]},
                                  allow_input_downcast = True)
    ...
    print '... generating and training'+sim_string+' n_option: '+ str(n_option)
    epoch = 0
    best_val_perf = 0
    val_perf = 0
    test_perf = 0       
    ...
    while (epoch < n_epochs):
        ...
        for i in xrange (n_train_batches):
            index =i
            din_onehot=Fdin_onehot(i)
            for i in xrange(din_onehot.shape[0]):
                for j in xrange (din_onehot.shape[2]):
                    if i == 0 and j==0:
                        Repeat=np.repeat(din_onehot[i,0,j,int(train_set_xout[i,j])],len(vocab)+1)
                        continue
                    temp=np.repeat(din_onehot[i,0,j,int(train_set_xout[i,j])],len(vocab)+1)
                    Repeat=np.row_stack([Repeat,temp])
            Repeat=Repeat.reshape(din_onehot.shape[0],1,din_onehot.shape[2],din_onehot.shape[3])    
            din=din_onehot-Repeat
            
            all_din1_indextemp=np.max(din,axis=3)
            all_din2_index=np.argsort(din,axis=3)
            fir_din1_index=np.argmax(all_din1_indextemp,axis=2)
