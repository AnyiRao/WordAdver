...
def loaddata_part():
    with open('save/I_high.pickle', 'rb') as file:
         tempx=cPickle.load(file)
         I=tempx #.astype(int)
#         print(Dinput_1[0][:10])
 
    with open('save/M_high.pickle', 'rb') as file:
             M=cPickle.load(file)
             
#    with open('save/Din_value.pickle', 'rb') as file:
#             Din_value=cPickle.load(file)
    
#    with open('save/Din_index.pickle', 'rb') as file:
#             Din_index=cPickle.load(file)
...
#    f0=theano.function([x],layer_printed)
    ...
    print 'This is a testing program: first train and then load adver data'
    epoch = 0
    best_val_perf = 0
    val_perf = 0
    test_perf = 0       
    ...
    while (epoch < n_epochs):
        ...
        train_set_xout= train_set[:,:img_h]# this is for test
...
#    train_set_y_adver = train_set_y[1000:2046]
    ...
    set_x_adver,set_y_adver =loaddata_part()

    Set_x_adver=np.tile(set_x_adver,(10,1))
    Set_y_adver=np.tile(set_y_adver,10)
    train_set_x_adver = Set_x_adver[000:1050]
    train_set_y_adver = Set_y_adver[000:1050]
