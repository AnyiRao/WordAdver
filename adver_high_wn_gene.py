...
if __name__=="__main__":
    print "loading data...",
    x = cPickle.load(open("mr_large.p","rb"))
...
#    f0=theano.function([x],layer_printed)
    ...
    Fdinput_1hot=theano.function([index], dinput_1hot,
          givens={
            x: train_set_x[index*batch_size:(index+1)*batch_size],
              y: train_set_y[index*batch_size:(index+1)*batch_size]},
                                  allow_input_downcast = True)
    ...
    din_value_sort=theano.function([index], din1temp,
          givens={
            x: train_set_x[index*batch_size:(index+1)*batch_size],
              y: train_set_y[index*batch_size:(index+1)*batch_size]},
                                  allow_input_downcast = True)
    ...
    din_index_sort=theano.function([index], din1_indextemp,
          givens={
            x: train_set_x[index*batch_size:(index+1)*batch_size],
              y: train_set_y[index*batch_size:(index+1)*batch_size]},
                                  allow_input_downcast = True)
...
#        din_value=din_value_sort(0)
#        din_value_reshape= din_value.reshape(50,60,25821)  
#        din_value_raw=din_value_reshape[0,:,:]
#        
#        din_index=din_index_sort(0)
#        din_index_reshape=din_index.reshape(50,60,25821)
#        din_index_raw=din_index_reshape[0,:,:]
        
#        dinput_1hot=Fdinput_1hot(0)
...
#                        print w_rep_index
                        if w_rep_index != None:
#                            print "this sentence changed ......"+str(m)
...
#                print "this sentence  No synonm"+str(m)
                    continue
                if w_rep_index == None or w_rep_index == 0:
                    continue
#                print "this sentence save"+str(m)
                    
                din_hightemp=din_index_reshape[m,dinput_1[m],:]
                din_high_order=np.where(din_hightemp==w_rep_index)[1]
                if din_high_order < (0.95*len(vocab)):
                    print "low gradient"+str(m)
                    continue
                if I == [] and M == []:
                     I=i
                     M=m
        ...
        print 'epoch '+ str(epoch)+ ' sort and replace time: ' + str(time.time()-start_time) +'secs'
...
# =============================================================================
    ...
    with open('save/I_high.pickle', 'wb') as file:
        model = I
        cPickle.dump(model, file)
#        print(model[0][:10])
    with open('save/M_high.pickle', 'wb') as file:
        model = M
        cPickle.dump(model, file)
#        print(model[0][:10])
    with open('save/adver_Train_set_x_adversingle_wn.pickle', 'wb') as file:
        model = Train_set_x_adversingle
        cPickle.dump(model, file)
#        print(model[0][:10])
        
    
    with open('save/adver_Train_set_y_adversingle_wn.pickle', 'wb') as file:
