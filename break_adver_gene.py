...
########
    ...
    dinput_2=T.lmatrix()
    j=T.lscalar()
    train_set_temp=T.set_subtensor((train_set_x[index*batch_size:(index+1)*batch_size]) \
                                [j:j+1,dinput_1[j]],dinput_2[j:j+1])
    f_train=theano.function([index,j,dinput_1,dinput_2],train_set_temp)
#########    
    ...
    while (epoch < n_epochs):
        ...
        dinput_1test=f1_printed(0)
        dinput_2test=f2_printed(0)
        
        train_set_xtest=Foutput_train_set_x(0)
        train_set_ytest=Foutput_train_set_y(0)
        
        #print word after replacement
        for i in xrange (batch_size):
            print word_idx_map.keys()[word_idx_map.values().index(dinput_2test[i])]
        # print the word be replaced
        for i in xrange (batch_size):
            wtemp=train_set_xtest[i,dinput_1test[i]]
            if wtemp == 0:
                print "NAN"
                continue
            print word_idx_map.keys()[word_idx_map.values().index(wtemp)]
        #print text
        for i in xrange (batch_size):
            print revs[i].get('text')
        ...
        for i in xrange (n_train_batches):
            ...
            train_set_xtest=Foutput_train_set_x(i)
            train_set_ytest=Foutput_train_set_y(i)
            Train_set_xtest.append(train_set_xtest)
            Train_set_ytest.append(train_set_ytest)
            for j in xrange (batch_size): 
                T.set_subtensor((train_set_x[index*batch_size:(index+1)*batch_size]) \
                                [j:j+1,dinput_1[j]],dinput_2[j:j+1])
                train_set_temp=f_train(i,j,dinput_1,dinput_2)
                Train_set_temp.append(train_set_temp)
             
        print 'epoch '+ str(epoch)+ ' replacing time: ' + str(time.time()-start_time) +'secs'
        
#        train_set_xtest1=Foutput_train_set_x(0)
#        train_set_ytest1=Foutput_train_set_y(0)
    ###############        
...
#            print '...save'
#            with open('save/advermodeltest.pickle', 'wb') as file:
#                model = params
#                cPickle.dump(model, file)
#                print(model[0][:10])
            
#            with open('save/adver_xtest.pickle', 'wb') as file:
#                model = new_data
#                cPickle.dump(model, file)
#                print(model[0])
            
#    return test_perf        
    return test_perf,dinput_1test,dinput_2test,Train_set_xtest,Train_set_ytest,\
                            Dinput_1,Dinput_2,Train_set_temp
...
#         #output changed
#         perf,dinput_1test,dinput_2test,train_set_xtest,train_set_ytest,\
#                             Dinput_1,Dinput_2,Train_set_temp = train_conv_net(datasets,
#                               U,
#                               lr_decay=0.95,
#                               filter_hs=[3,4,5],
#                               conv_non_linear="relu",
...
# =============================================================================
    ...
    perf,dinput_1test,dinput_2test,Train_set_xtest,Train_set_ytest,\
                            Dinput_1,Dinput_2,Train_set_temp = train_conv_net(datasets,
                              ...
                              conv_non_linear="relu",
                              hidden_units=[100,2], 
                              shuffle_batch=True, #here is changed
                              n_epochs=1, #here is changed
                              sqr_norm_lim=9,
                              non_static=non_static,
                              batch_size=50,
